...
weight_path = 'params/unet_raft4.pth'  # 权重地址
data_path = r'C:\Data_Sentinel2\F\cut_256'  # 数据集地址
save_path = 'train_image'
if __name__ == '__main__':  # 加载数据集
    # num_classes = 2 + 1  # +1是背景也为一类
    data_loader = DataLoader(MyDataset(data_path), batch_size=1, shuffle=True)  # 数据加载器、实例化数据集、batch_size看电脑性能
    ...
    opt = optim.Adam(net.parameters())  # 优化器
    ...
    while epoch < 900:
        ...
        for i, (image, segment_image) in enumerate(data_loader):  # 元组，为了得到下标i
            ...
            if i % 10 == 0:  # 每隔五次打印一下损失值
                print(f'{epoch}-{i}-train_loss===>>{train_loss.item()}')  # 轮次--i--train_loss
                with open('loss_value/loss_value_4.txt', 'a') as f:  # 使用追加模式打开文件
                    # f.write(f'{epoch}-{i}: {train_loss.item()}\n')  # 写入epoch, iteration和损失值
                    f.write(f'{train_loss.item()}\n')  # 写入epoch, iteration和损失值
            # 保存权重
            if i % 50 == 0:  # 每隔50次保存权重
